...
const CreatePage = ({ navigation }) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // async storage'dan books verisini al, bunu JS formatına çevir, eğer yoksa da boş dizi olarak al
      let books = await AsyncStorage.getItem('books');

      books = books ? JSON.parse(books) : []

      // bu async storagedan gelen books verisine yukarda oluşan kitabı ekle
      books = [...books, uploadItem];

      // sonrasında async storagedaki veriyi güncelle
      await AsyncStorage.setItem('books', JSON.stringify(books));

      dispatch(addBook(uploadItem));

      Alert.alert('Başarılı',
        "Kitabı başarıyla yayınladınız."
      )
